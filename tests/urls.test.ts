 ...
 *
 * It is generated by `src/pages/sitemap.xml.ts`, which finds pages with
 * `import.meta.glob`. This is the check that discovery keeps finding all of
 * them: a page added outside the `<name>/index.astro` convention is missed
...
describe('sitemap contents', () => {
  ...
  const crawlable = expectedUrls.filter(
    (url) => !['/404.html', '/feed.xml', '/sitemap.xml', '/robots.txt'].includes(url),
  );
